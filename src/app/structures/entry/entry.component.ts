...
export class EntryComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    let sectionId = parseInt(this._route.snapshot.params['section_id'], 10);
    let itemId = parseInt(this._route.snapshot.params['item_id'], 10);
  ...
  private getSectionAndItem(sectionId: number, itemId: number) {
    ...
    this._structureService.getStructure().then(() => {
      ...
      const knowledgeBaseContent = <HTMLInputElement>(
        document.querySelector('.pia-knowledgeBaseBlock-searchForm input')
      );
