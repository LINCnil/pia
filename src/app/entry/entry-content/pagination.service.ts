...
export class PaginationService {
  ...
  setPagination(sectionId: number, itemId: number) {
    this.previousLink = []
    this.nextLink = []
    this.hasPreviousLink =
      !(sectionId === 1 && itemId === 1) &&
      !(sectionId === 4 && itemId === 4) &&
      !(sectionId === 4 && itemId === 5)
    this.hasNextLink = !(
      sectionId === 4 &&
      (itemId === 3 || itemId === 4 || itemId === 5)
    )
  ...
  getNextSectionItem(status_start: number, status_end: number) {
    ...
    const itemStatus = Object.keys(this._sidStatusService.itemStatus)
      .sort()
      .reduce((r, k) => ((r[k] = this._sidStatusService.itemStatus[k]), r), {})
