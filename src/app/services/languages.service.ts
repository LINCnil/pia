...
export class LanguagesService {
   ...
   */
  initLanguages() {
    this._translateService.addLangs(['en', 'cs', 'it', 'nl', 'fr', 'pl', 'de', 'es', 'no', 'el', 'pt', 'fi', 'ro']);
  ...
  getOrSetCurrentLanguage() {
    ...
    if (language && language.length > 0) {
      ...
      language = browserLang.match(/en|cs|it|nl|fr|pl|de|es|no|el|pt|fi|ro/) ? browserLang : 'fr';
      this._translateService.use(language);
    }
